<script>
    ...
    function switchMode() {
        const isBright = parseInt(localStorage.isBright)
        const mode = isBright ? dark : bright

        Object.keys(mode).forEach((k) => {
            document.querySelector(':root').style.setProperty(k, mode[k])
        })

        if (isBright) {
            localStorage.isBright = 0
        } else {
            localStorage.isBright = 1
        }
    }

    function loadMode() {
        const isBright = parseInt(localStorage.isBright)
        const mode = isBright ? bright : dark
        Object.keys(mode).forEach((k) => {
            document.querySelector(':root').style.setProperty(k, mode[k])
        })
    }
...
<div class="dark-switch" on:click={switchMode}>
